...
namespace mig 
{
	...
	PBYTE FindPatter1n(PVOID base, DWORD size, LPCSTR pattern, LPCSTR mask) {
	...
	PBYTE FindPattern11(LPCSTR pattern, LPCSTR mask) {
		MODULEINFO info = { 0 };
		GetModuleInformation(GetCurrentProcess(), GetModuleHandle(0), &info, sizeof(info));

		return FindPatter1n(info.lpBaseOfDll, info.SizeOfImage, pattern, mask);
	}
	template <typename Ret, typename... Args>
	Ret SpoofCall1(Ret(*fn)(Args...), Args... args) {
		static PVOID trampoline = nullptr;
		if (!trampoline) {
			trampoline = FindPattern11("\xFF\x27", "xx");
			if (!trampoline) {
		...
		return _SpoofCallInternal::Remapper<sizeof...(Args), void>::template Call<Ret, Args...>(&FC::_spoofer_stub, &params, args...);
	}
}